  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="fade-in" style={{ textAlign: 'center' }}>
        <div style={{ width: 40, height: 40, borderRadius: '9px', background: 'rgba(22,163,74,0.08)', border: '1px solid rgba(22,163,74,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M3 8l3.5 3.5L13 4.5" stroke="#16a34a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
        <p style={{ fontSize: '14px', color: 'var(--text)', fontWeight: 500, letterSpacing: '-0.01em' }}>Account created</p>
        <p style={{ fontSize: '12px', color: 'var(--text-3)', marginTop: '4px' }}>Redirecting to workspace…</p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex' }}>
      ...
      <div style={{
        ...
        <div>
          <p className="mono" style={{ fontSize: '10px', color: 'var(--text-4)', letterSpacing: '0.07em', marginBottom: '18px' }}>{ '{ new account }' }</p>
          <h1 style={{ fontSize: '28px', fontWeight: 300, color: 'var(--text)', lineHeight: 1.25, letterSpacing: '-0.03em' }}>
            Start building<br />
            <span style={{ color: 'var(--text-3)' }}>with your team.</span>
          </h1>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
        <div className="fade-in" style={{ width: '100%', maxWidth: '320px' }}>
          <div style={{ marginBottom: '30px' }}>
            <h2 style={{ fontSize: '19px', fontWeight: 500, color: 'var(--text)', marginBottom: '5px', letterSpacing: '-0.02em' }}>Create account</h2>
            <p style={{ fontSize: '13px', color: 'var(--text-3)' }}>Join your team's workspace</p>
          </div>

          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '11px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-2)', marginBottom: '5px' }}>Full name</label>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Jane Smith" required className="tt-input" />
            </div>
            <div>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-2)', marginBottom: '5px' }}>Password</label>
            ...
            >
              {loading
                ? <><span style={{ width: 13, height: 13, border: '1.5px solid rgba(247,246,243,0.4)', borderTop: '1.5px solid rgba(247,246,243,0.9)', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.7s linear infinite' }} /> Creating…</>
                : 'Create account →'
              }
            </button>
          </form>
          ...
          <p style={{ marginTop: '22px', fontSize: '12px', color: 'var(--text-3)', textAlign: 'center' }}>
            ...
            <Link to="/login" style={{ color: 'var(--text-2)', textDecoration: 'underline', textUnderlineOffset: '3px' }}>
              Sign in
            </Link>
          </p>
        </div>
